...
/* In this example, there is a thread waiting for a synchronization signal from
another thread before it start processing Task synchronization could also be
achieved with `xSemaphoreTake`, but FreeRTOS suggest using task notifications as
a faster and more lightweight alternative.
*/
...
int comp_task_notification_entry_func(int argc, char **argv) {
  ...
  xTaskCreatePinnedToCore(notification_rcv_func, NULL, 8192, NULL, TASK_PRIO_3,
                          &rcv_task_hdl, tskNO_AFFINITY);
  xTaskCreatePinnedToCore(notification_send_func, NULL, 8192, rcv_task_hdl,
