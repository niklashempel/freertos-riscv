...
uint32_t cct_ElapsedTimeMs(uint32_t start, uint32_t end, uint32_t cpu_hz) {
  ...
  return (cycles + (cycles_per_ms / 2)) /
         cycles_per_ms; // "add half and divide" rounding technique
}
...
int comp_time_measurement_entry_func(int argc, char **argv) {
  ...
  uint32_t elapsed_time_ms =
      cct_ElapsedTimeMs((uint32_t)start, (uint32_t)end, cpu_hz);
  printf("[%s] Elapsed cycles: %" PRIu32 ", Elapsed time: %" PRIu32
         " ms (expected ~2000)\n",
