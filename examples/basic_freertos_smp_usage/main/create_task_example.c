...
#define SPIN_ITER                                                              \
  350000 // actual CPU cycles consumed will depend on compiler optimization
#define CORE0 0
...
static void spin_task(void *arg) {
  ...
  while (!timed_out) {
    ...
    // consume some CPU cycles to keep Core#0 a little busy, so task3 has
    // opportunity to be scheduled on Core#1
...
// Creating task example: show how to create pinned and unpinned tasks on CPU
// cores
int comp_creating_task_entry_func(int argc, char **argv) {
  ...
  xTaskCreatePinnedToCore(spin_task, "pinned_task0_core0", 4096,
                          (void *)task_id0, TASK_PRIO_3, NULL, CORE0);
  xTaskCreatePinnedToCore(spin_task, "pinned_task1_core0", 4096,
                          (void *)task_id1, TASK_PRIO_3, NULL, CORE0);
  xTaskCreatePinnedToCore(spin_task, "pinned_task2_core1", 4096,
                          (void *)task_id2, TASK_PRIO_3, NULL, CORE1);
  // Create a unpinned task with xCoreID = tskNO_AFFINITY, which can be
  // scheduled on any core, hopefully it can be observed that the scheduler
  // moves the task between the different cores according to the workload
  xTaskCreatePinnedToCore(spin_task, "unpinned_task", 4096, (void *)task_id3,
                          TASK_PRIO_2, NULL, tskNO_AFFINITY);
