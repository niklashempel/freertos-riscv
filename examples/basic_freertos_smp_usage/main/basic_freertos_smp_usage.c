...
static void register_creating_task(void) {
  const esp_console_cmd_t creating_task_cmd = {
      .command = "create_task",
      .help = "Run the example that demonstrates how to create and run pinned "
              "and unpinned tasks",
      .hint = NULL,
...
static void register_queue(void) {
  const esp_console_cmd_t queue_cmd = {
      .command = "queue",
      .help = "Run the example that demonstrates how to use queue to "
...
static void register_lock(void) {
  const esp_console_cmd_t lock_cmd = {
      .command = "lock",
      .help = "Run the example that demonstrates how to use mutex and spinlock "
...
static void register_task_notification(void) {
  const esp_console_cmd_t task_notification_cmd = {
      ...
      .help = "Run the example that demonstrates how to use task notifications "
...
static void register_batch_proc_example(void) {
  const esp_console_cmd_t batch_proc_example_cmd = {
      ...
      .help = "Run the example that combines queue, mutex, task notification "
              "together",
      .hint = NULL,
...
static void register_periodic_blink_example(void) {
  const esp_console_cmd_t periodic_blink_example_cmd = {
      .command = "periodic_blink",
      .help = "Run the example that demonstrates how to run periodic tasks",
      .func = &comp_periodic_blink_entry_func};
...
static void register_priority_inheritance_example(void) {
  const esp_console_cmd_t priority_inheritance_example_cmd = {
      .command = "priority_inheritance",
      .help =
          "Run the example that demonstrates priority inheritance mechanism",
      .func = &comp_priority_inheritance_entry_func};
...
void spin_idle(int ms) {
  ...
  while (xTaskGetTickCount() - xTimeOnEntering < pdMS_TO_TICKS(ms)) {
    // Do nothing
  }
}
